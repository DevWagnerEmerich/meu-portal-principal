...
const findUserByEmail = async (email) => {
    return await db('users')
        .select('id', 'username', 'email') // Added email to select just in case
        .where('email', email)
        .first();
};
...
const disableWelcomeModal = async (userId) => {
    await db('users')
        .where('id', userId)
        .update({ show_welcome_modal: 0 }); // booleans in knex/sqlite/pg are handled, usually 0/1 or false/true. Knex handles false -> 0/false
};
